...
def log_plate(number_plate, object_label="vehicle", confidence=0.0):
    ...
    for r in results.boxes:
        ...
        if cls_name in ['car', 'bus', 'truck', 'motorbike'] and conf > 0.5:
            ...
            for bbox, text, score in ocr_results:
                if len(text) >= 4:  # crude filtering
                    number_plate = text.upper()
                    cv2.putText(frame, f"{number_plate} ({score:.2f})", (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                    log_plate(number_plate, cls_name, conf)
                    break
